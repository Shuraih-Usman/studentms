...
if($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if($action === 'add') {
        ...
        $response = $doctor->add($data, $image);
        ...
        foreach($results as $row) {
            ...
            $action = '<button type="button" class="btn btn-primary edit mx-1" data-first_name="'.$row['first_name'].'" data-last_name="'.$row['last_name'].'" data-specialization="'.$row['specialization'].'" data-address="'.$row['address'].'" data-gender="'.$row['gender'].'" data-image="'.$row['image'].'" data-phone="'.$row['phone_number'].'" data-email="'.$row['email'].'" data-username="'.$row['username'].'" data-id="'.$row['id'].'"><i class="fa fa-pen"></i> </button>';

            $action .= $row['status'] == 1 ? '<button type="button" class="btn btn-warning deactivate mx-1" data-id="'.$row['id'].'"><i class="fa fa-trash"></i> </button>' : '<button type="button" class="btn btn-success activate mx-1" data-id="'.$row['id'].'"><i class="fa fa-arrow-alt-circle-up"></i> </button>';
